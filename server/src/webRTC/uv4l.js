...
var signalling_server_hostname = location.hostname || "192.168.1.99"
var signalling_server_address =
...
addEventListener("DOMContentLoaded", function () {
  ...
  var cast_not_allowed =
    !("MediaSource" in window) || location.protocol !== "https:"
  if (cast_not_allowed || !isFirefox) {
    if (document.getElementById("cast_tab"))
      document.getElementById("cast_tab").disabled = true
    if (cast_not_allowed) {
      // chrome supports if run with --enable-usermedia-screen-capturing
...
var recordedBlobs
var pcConfig = {
  /*sdpSemantics : "plan-b"*,*/ iceServers: [
    {
      urls: [
...
function start_call(onopen) {
  if ("WebSocket" in window) {
    ...
    function call(stream) {
      ...
      var request = {
        ...
        options: {
          ...
          vformat: 25, //document.getElementById("remote_vformat").value,
    ...
    ws.onopen = function () {
      ...
      var cast_tab = false //document.getElementById("cast_tab")
      // ? document.getElementById("cast_tab").checked
      // : false
      ...
      var echo_cancellation = false //document.getElementById("echo_cancellation")
      var localConstraints = {}
      if (cast_mic) {
        if (echo_cancellation)
          localConstraints["audio"] = isFirefox
      ...
      else localConstraints["video"] = false
        ...
        if (navigator.getUserMedia) {
          navigator.getUserMedia(
            ...
            function (error) {
              stop()
              alert(
                "An error has occurred. Check media device, permissions on media and origin."
              )
              console.error(error)
            }
          )
...
function toKeyCode() {
  ...
  if (
    datachannel &&
    document.getElementById("keypresssend").checked &&
    keysArray.length
  ) {
    var keycodes = {
      keycodes: keysArray,
...
export { start_call, send_message }
